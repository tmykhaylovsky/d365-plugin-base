# Ops.Crm.Models — Early-Bound Classes

This is a [Shared Project](https://learn.microsoft.com/en-us/xamarin/cross-platform/app-fundamentals/shared-projects) (`Ops.Crm.Models.shproj`). It contains no assembly of its own — consuming projects import `Ops.Crm.Models.projitems` and the source files are compiled directly into them.
...
~/.dotnet/tools/pac modelbuilder build \
  --settingsTemplateFile models/builderSettings.json \
  --outdirectory models
```

The tool overwrites all files it manages (`Entities/`, `OptionSets/`, `CrmServiceContext.cs`, `EntityOptionSetEnum.cs`). After running, sync `Ops.Crm.Models.projitems` with any files that were added or removed (see below).
...
3. Add the new `Entities\<tablename>.cs` to `Ops.Crm.Models.projitems`:
...
Then add all new files that appear in `OptionSets\` to `Ops.Crm.Models.projitems`.
...
Use a trailing wildcard to match by prefix (e.g. `"ops_*"`). Message files land in `Messages/`. After generation, add them to `Ops.Crm.Models.projitems`:
