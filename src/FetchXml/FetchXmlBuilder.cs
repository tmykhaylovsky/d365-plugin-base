...
namespace Ops.Crm.Shared.FetchXml
{
    public class FetchXmlBuilder : EntityBuilder<FetchXmlBuilder>
    {
        ...
        public FetchXmlBuilder(bool aggregate = false, int? count = null, bool distinct = false)
        {
            ...
            if (aggregate) builder.Append(" aggregate='true'");
            if (count != null) builder.Append($" count='{count}'");
            if (distinct) builder.Append(" distinct='true'");
        ...
        public FetchXmlBuilder WithEntity(string logicalName, string alias = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(alias)) builder.Append($" alias='{alias}'");
            builder.Append(">");
        ...
        {
            _orders.Add(isAlias
                ? $"<order alias='{attribute}' descending='{descending.ToString().ToLower()}' />"
                : $"<order attribute='{attribute}' descending='{descending.ToString().ToLower()}' />");
            return this;
        }
        ...
        public FetchExpression Build()
        {
            ...
            foreach (var attribute in _attributes) builder.Append(attribute);
            foreach (var order in _orders) builder.Append(order);
            foreach (var filter in _filters) builder.Append(filter.Build());
            foreach (var linkEntity in _linkEntities) builder.Append(linkEntity.Build());
